...
class StreamEvent:
    """One incremental event from a streaming call."""

    kind: str  # "text_delta" | "thinking_delta" | "done"
    text: str = ""
...
class LLMClient:
    ...
    def _request(
        ...
        """Build kwargs and create the completion (streaming or not)."""
        kwargs: dict[str, Any] = {
    ...
    def chat(
        ...
        resp = self._request(messages, tools, temperature, stream=False)
        choice = resp.choices[0]
        result = ChatResult(
            message=Message.from_openai(choice.message.model_dump(exclude_none=True)),
    ...
    def chat_stream(
        ...
        """
        stream = self._request(messages, tools, temperature, stream=True)
